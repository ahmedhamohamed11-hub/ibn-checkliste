  ...
  const handleDuplicate = async (project: ProjectWithStats) => {
    ...
    if (origParticipants && origParticipants.length > 0) {
      await supabase.from('project_participants').insert(
        origParticipants.map(p => ({ project_id: newProject.id, user_name: p.user_name }))
      )
    }
...
>
            ...
            {filtered.map(project => (
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                  ...
                  {project.archived && (
                    <span style={{ fontSize: '11px', background: 'rgba(245,158,11,0.15)', color: '#f59e0b', border: '1px solid rgba(245,158,11,0.3)', borderRadius: '99px', padding: '2px 8px', whiteSpace: 'nowrap', fontWeight: 600 }}>
                      Archiviert
                    </span>
                  )}
                </div>
      ...
      {showCreateModal && (
        <CreateProjectModal
          ...
          onCreated={(id) => { setShowCreateModal(false); router.push(`/project/${id}`) }}
          userName={userName!}
        />
      )}
